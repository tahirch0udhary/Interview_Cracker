...
async function generate(prompt, apiKey, responseSize = 'medium', history = []) {
  try {
    // Accept model and temperature as arguments (default fallback for backward compatibility)
    const args = Array.from(arguments);
    let modelName = 'gemini-2.5-flash';
    let temperature = 1.0;
    if (args.length > 4 && typeof args[4] === 'string') modelName = args[4];
    if (args.length > 5 && typeof args[5] === 'number') temperature = args[5];
